...
public class Server {
	...
	public static void addUrls() {
		...
		app.get(
			STUDENT_PROFILE_URL_PREFIX + "/:studentId",
			ctx -> {
				Handlers.studentProfile(ctx);
			}
		);
		app.get(
			COURSE_URL_PREFIX + "/:code/:classCode",
			ctx -> {
				Handlers.singleCourse(ctx);
			}
		);
		app.get(
			CHANGE_PLAN_URL_PREFIX + "/:studentId",
			ctx -> {
				Handlers.changePlan(ctx);
			}
		);
		app.get(
			PLAN_URL_PREFIX + "/:studentId",
			ctx -> {
				Handlers.plan(ctx);
			}
		);
		app.get(
			SUBMIT_URL_PREFIX + "/:studentId",
			ctx -> {
				Handlers.submit(ctx);
			}
		);
		...
		// POST methods
		app.post(
			ADD_COURSE_URL_PREFIX + "/:code/:classCode",
			ctx -> {
				Handlers.addCourse(ctx);
			}
		);
		app.post(
			REMOVE_COURSE_URL_PREFIX + "/:code/:classCode",
			ctx -> {
